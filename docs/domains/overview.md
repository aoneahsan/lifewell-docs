...
[Addresses that do not move](../reference/addresses.md).

An area is a product idea. A prefix is a historical fact. Never read one as the other.
...
- the menu stops listing it;
- anything you recorded is kept — nothing is deleted, now or later;
- any page in it still opens if you go to it directly, with one line at the top saying why it is not in the
  menu.

There is no Save button on that panel. A switch that needs a Save is lying about what it did, so each flip
writes immediately and the switch itself is the acknowledgement. If a write fails, the switch moves back and
says so.

## Hiding an area is not deleting it
...
## What decides which screens appear

Two answers, and nothing else:

1. **Which areas you have on** — the six switches above.
2. **Who you are** — one profile field that decides whether six life-stage screens are listed. See
   [The first run](../getting-started/first-run.md#the-one-answer-that-changes-what-you-see).

A person meeting both finds the same sentence in both places, which is deliberate.
